...
class ImageAndPriorSeqData(ImageData):
    ...
    def next_batch(self):
        ...
        # print start, '---', end
        if self.crop_size:
            batch_x = np.zeros([self.seq_size, self.crop_size, self.crop_size, 4], dtype=np.float32)
            batch_y = np.zeros([self.seq_size, self.crop_size, self.crop_size, 1], dtype=np.float32)
        else:
            batch_x = np.zeros([self.seq_size, self.image_size, self.image_size, 4], dtype=np.float32)
...
class ImageAndPriorSeqBboxData(ImageData):
    ...
    def next_batch(self):
        ...
        batch_bbx = np.zeros([self.seq_size, 4])
        count = 0
        for index in range(start, end):
            # print label_path
            ...
            for i in range(len(images_path)):
                ...
                # plt.show()
                # center_x = bbox_crop[0] + (bbox_crop[2] - bbox_crop[0]) / 2
                # center_y = bbox_crop[1] + (bbox_crop[3] - bbox_crop[1]) / 2
